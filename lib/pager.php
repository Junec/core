...
class core_pager{
    ...
    public function pageList($params){
        ...
        if($pagenums>0){
            ...
            $list .= "<a href='".$this->_geturl($_params,'begin',true,$params['url_mode'])."'>".$this->firstName."</a><a href='";
            if($page>1){
                $list .= $this->_geturl($_params,'prev',true,$params['url_mode'])."' id='pager_prev'>".$this->prevName."</a>";
            }else{
                $list .= "#' type='normal' >".$this->prevName."</a>";
            }
            #$list .= $page>10?"<a href='".$this->_geturl($_params,'begin',true,$params['url_mode'])."'>1...</a>":'';
            for($i=$b;$i<=$e;$i++){
                $list .= "<a href='".$this->_geturl($_params,$i,true,$params['url_mode'])."'";
            ...
            $list .= "<a href='";
            if($page<$pagenums){
                $list .= $this->_geturl($_params,'next',true,$params['url_mode'])."'  id='pager_next'>".$this->nextName."</a>";
            }else{
                $list .= "#' type='normal'>".$this->nextName."</a>";
            }
            $list .= "<a href='".$this->_geturl($_params,'end',true,$params['url_mode'])."'>".$this->lastName."</a></div>";
        }
    ...
    public function get($params){
        ...
        $number = array();
        if($pagenums>0){
            $list .= $page>10?$number[] = array('num'=>$this->_geturl($_params,'begin',false)):'';
            for($i=$b;$i<=$e;$i++){
                $number[] = array('num'=>$this->_geturl($_params,$i,false),'active'=>$page==$i?true:false);
            }
            $list .= $page<=($pagenums-10)?$number[] = array('num'=>$this->_geturl($_params,'end',false)):'';
        }
    ...
    private function _geturl($_params,$sign,$isparseurl = true,$url_mode = ''){
        switch($sign){
            case 'begin':
                $go_page = 1;
            break;
